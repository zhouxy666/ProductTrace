...
class Seeyon extends CI_Model{
		...
		if(!isset($result['status'])){
			...
			if($result['status']==0){
				if(isset($result['message'])){
                    show_error($result['message'],401,"第三方系统返回错误，原因如下:");
				}else{
                    show_error("未知原因错误发生",401,"第三方系统返回错误，原因如下:");
				}
			}
		}
	}
	...
	public function getToken(){
		try{
			...
			if(!empty($output)){
                ...
                show_error("无法获取返回结果",401,"调用获取tokenAPI...发生错误，原因如下:");
			}
		}catch(Exception $e){
            show_error($e->getMessage(),401,"调用获取tokenAPI...发生错误，原因如下:");
		}
	}
	...
	*/
	...
	public function exportForm($templateCode,$beginDateTime,$endDateTime){
		try{
			...
			if(!empty($output)){
                ...
                show_error("无法获取返回结果",401,"调用导出无流程表单数据API...发生错误，原因如下:");
			}
		}catch(Exception $e){
            show_error($e->getMessage(),401,"调用导出无流程表单数据API...发生错误，原因如下:");
		}
	}

    /**
     * 导入无流程 业务生成器 表单数据 API
     * @param $templateCode
     * @param $loginName
     * @param $dataXml
     * @return array
     */
    public function importForm($templateCode,$loginName,$dataXml){
        try{
            $url    = $this->restUrl."form/import/$templateCode?token=$this->token";
            $data   = array("loginName"=>$loginName,"dataXml"=>$dataXml);
            $output = postData($url,$data);
            $this->check401($output);
            ...
            if(!empty($output)){
                ...
                show_error("无法获取返回结果",401,"调用导入无流程表单数据API...发生错误，原因如下:");
            }
        }catch(Exception $e){
            show_error($e->getMessage(),401,"调用导入无流程表单数据API...发生错误，原因如下:");
        }
    }
     ...
     * @return array
     */
    public function getOrgAccounts(){
        try{
            $url = $this->restUrl."orgAccounts?token=$this->token";
            $output = getData($url);
            $this->check401($output);

            if(!empty($output)){
                $data = json_decode($output,null,512,JSON_BIGINT_AS_STRING);
                return $data;
                /*
                return array(
                    'data'=>$data,
                ...
                */
                ...
                show_error("无法获取返回结果",401,"调用【获取所有单位信息】接口...发生错误，原因如下:");
            }
        }catch(Exception $e){
            show_error($e->getMessage(),401,"调用【获取所有单位信息】接口...发生错误，原因如下:");
        }
    }
    /**
        ...
        try{
            ...
            if(!empty($output)){
                ...
                if(isset($obj->success) && $obj->success==true && !empty($obj->successMsgs[0]->ent)) {
                    ...
                    show_error($output,401,"创建部门失败，原因如下:");
                }
            }else{
                show_error("无法获取返回结果",401,"调用【创建部门】接口...发生错误，原因如下:");
            }
        }catch(Exception $e){
            show_error($e->getMessage(),401,"调用【创建部门】接口...发生错误，原因如下:");
        }
    }


    /**
     * 创建人员 API 单位ID 部门ID 岗位ID 用户名 密码
     * @param $orgAccountId
     ...
     * @return array
     */
    public function createOrgMember($orgAccountId,$orgDepartmentId,$orgPostId,$orgLevelId,$name,$loginName,$password){
        try{
            $data = "{\"orgAccountId\":$orgAccountId,\"orgDepartmentId\":$orgDepartmentId,\"orgPostId\":$orgPostId,\"orgLevelId\":$orgLevelId,\"name\":\"$name\",\"loginName\":\"$loginName\",\"password\":\"$password\"}";
            ...
            if(!empty($output)){
                ...
                if(isset($obj->success) && $obj->success==true && !empty($obj->successMsgs[0]->ent)) {
                    ...
                    show_error("该账号已经被注册",401,"创建人员失败，原因如下:");
                }
            }else{
                show_error("无法获取返回结果",401,"调用【创建人员】接口...发生错误，原因如下:");
            }
        }catch(Exception $e){
            show_error($e->getMessage(),401,"调用【创建人员】接口...发生错误，原因如下:");
        }
    }
}
 ...
 * @return mixed
 */
function putData($url,$data,$isJson=false,$method='put'){
    if(!$isJson){
        $json = json_encode($data);
    }else{
        $json = $data;
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, $json); //设置请求体，提交数据包
	$output = curl_exec($ch);//运行curl
	curl_close($ch);
	return $output;
}

function putData2($url,$method){
    $ch = curl_init();//初始化curl
    curl_setopt($ch, CURLOPT_URL,$url);//抓取指定网页
    curl_setopt($ch, CURLOPT_TIMEOUT, 60); //设置超时
    curl_setopt($ch, CURLOPT_HEADER, 0);//设置header
